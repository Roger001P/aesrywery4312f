...
class UserItemGcn(nn.Module):
    ...
    def forward(self, user_emb, entity_emb, interact_mat, mess_dropout=True, node_dropout=False):
        """node dropout"""
        if node_dropout:
        ...
        user_agg = user_emb
        item_agg = entity_emb[:self.n_items, :]
        user_ui_emb = torch.zeros(size=user_emb.size(), device='cuda:0')
        item_ui_emb = torch.zeros(size=entity_emb[:self.n_items, :].size(), device='cuda:0')
        for i in range(self.n_hops):
            ...
            user_ui_emb = torch.add(user_agg, user_ui_emb)
            item_ui_emb = torch.add(item_agg, item_ui_emb)
...
class KgAttention(nn.Module):
    ...
    def forward(self, entity_emb, edge_index, edge_type, edge_emb, mess_dropout):
        ...
        for i in range(self.n_hops):
            ...
            entity_kg_emb = torch.add(entity_kg_emb, agg_emb)
